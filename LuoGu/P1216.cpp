...
int main(){
    ...
    clock_t dfs_begin, dfs_end;
    clock_t dp_begin, dp_end;
    dfs_begin = clock();
    cout << dfs(1,1) << '\n';
    dfs_end = clock();
    dp_begin = clock();
    cout << dp(n) << '\n';
    dp_end = clock();
    cout << "dfs_time = " << double(dfs_end-dfs_begin)/CLOCKS_PER_SEC << '\n';
    cout << "dp_time = " << double(dp_end-dp_begin)/CLOCKS_PER_SEC << '\n';
}