### 总
`笛卡尔树`,`重链剖分`,`KMP`,`卢卡斯(快速求组合数，杨辉三角奇偶性)`,`统计1~n之间所有数数位和`,`辛普森公式求弧长公式定积分`,`kruskal重构树`,`区间修改线段树`,`矩阵乘法`,`LCA`,`分块`,`动态开点线段树`,`可持久化线段树（主席树，[l, r] 区间第k小）`,`字符串哈希`,`st表`,`并查集（启发式合并）`,`数学`,`dp`,`拓欧`,`tarjan求强连通分量`,`建虚树`,`Manacher(回文串匹配)`,`AC自动机`,`线性筛`,`拓扑排序`,`分组背包`,`Floyd最短路（含修改边权）`,`根号分块`,`tricks`
...
- 一棵无根树，现在以z为根
 $$ lca(x, y) = 以树上任意一点为根, diff(lca(x, y), lca(x, z), lca(y, z)) $$
