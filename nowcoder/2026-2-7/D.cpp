...
void solve() {
    ...
    */
    ...
    auto check = [&](int from, int to) -> int {
        ...
        // cout << sx << ' ' << sy << ' ' << fx << ' ' << fy << '\n';
        vector<vector<int>> mp = Mp, vis(n + 1, vector<int> (m + 1, 0)), dis(n + 1, vector<int> (m + 1, inf));
        auto bfs = [&](int x, int y, int v) -> void {
            queue<node> que;
            que.push({x, y, 0});
            dis[x][y] = 0;
            while (siz(que)) {
                ...
                for (int k = 0;k < 4;k ++) {
                    ...
                    if (!vaild(nx, ny) || dis[nx][ny] != inf || (mp[nx][ny] && mp[nx][ny] != v)) continue;
                    if (dis[nx][ny] > d + 1) {
                        dis[nx][ny] = d + 1;
                        que.push({nx, ny, d + 1});
                    }
